...
| `mic.cjs`          | OBS password                                                                                | Voice notes with a stale microphone id (found by name) and with an unknown microphone (Windows default + warning), from the hotkey and the button; a button note holds the voice-note key only with "holdHotkeyFromButtons"                                                                                                                 |
| `ptt.cjs`          | nothing (no OBS)                                                                            | Companion push-to-talk buttons: key held and released on the PC (F16, and End through SendInput), another device can't release it, a disconnected device releases it, unknown or unset buttons refused, saving keys doesn't disconnect devices                                                                                              |
...
`review.cjs`, `sessions.cjs`, `transcribe.cjs`, `mic.cjs` and `ptt.cjs` run in an isolated app instance (own settings folder
