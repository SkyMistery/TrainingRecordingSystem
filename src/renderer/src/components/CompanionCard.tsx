...
import type { AppState, CompanionSettings } from '@shared/types'
        ...
        {settings.enabled && (
          <>
            <div className="flex flex-wrap gap-2">
              ...
              <Button
                variant="outline"
                onClick={() => void window.api.openCompanionInBrowser()}
            ...
            {settings.lan && (
              <div className="flex flex-wrap items-start gap-5">
                {info.qr && (
                  <img src={info.qr} alt="QR code to pair a device" className="size-48 rounded-md bg-white p-2" />
                )}
                <div className="flex min-w-64 flex-1 flex-col gap-2 text-sm">
                  ...
                  <p className="text-xs text-muted-foreground">
                    The link contains a secret code: share it only with your own devices. Windows may ask to allow the
                    app on private networks — allow it, or the tablet can’t connect.
                  </p>
                </div>
              </div>
            )}
